...
def _detect_promotion_signals(code: str, name: str) -> list[dict]:
    ...
    # 长词优先匹配，避免 "老师推荐" 同时命中 "推荐" 造成重复计数
    matched_items: set[int] = set()
    for kw in sorted(PROMOTION_KEYWORDS, key=lambda k: -len(k["term"])):
        hits = [t for t in titles if kw["term"].lower() in t.lower()]
        if hits and kw["term"] not in "".join(
            titles[i] for i in matched_items if i < len(titles)
        ):
            pass
        if hits:
            signals.append({
                ...
                "evidence": f"命中关键词「{kw['term']}」: {hits[0][:60]}",
            })
    return signals
...
# ════════════════════════════════════════════════════════════════
# 以下为 unit_economics / value_creation_plan（移植自 Gateway
# src/tools/unit_econ.ts / src/tools/value_plan.ts，纯规则零 LLM）
# ════════════════════════════════════════════════════════════════
import json
